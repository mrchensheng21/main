...
To begin, let's download sample interviewers availability and interviewees details in comma-separated value (.csv) files and import the data into Scheduler.
...
time slot (at the leftmost column on the same row where the 1 is located), otherwise is 0. The imported interviewers details will also be displayed
...
in one of the tables in the `Schedule tab` depending on which slot is allocated to him/her.
...
at the target destination you specify in `file path`!

That's all for the quick start and hope you enjoy using Scheduler!
