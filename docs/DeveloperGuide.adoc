...
*Step 1. Once User imports a new .csv file, it will call the RefreshListener Interface. RefreshListenerManager will then take in the new titles and
list of ObservableList of interviewee.

*Step 2. The RefreshListenerManager will call the MainWindow to refresh the ScheduleViewPanel.

*Step 3. ScheduleViewPanel will firstly clear all the data and input the updated data from the .csv file.
